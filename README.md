...
3. Update the `supabaseUrl` and `supabaseAnonKey` in `src/services/supabase.ts` with your project credentials
4. Run the SQL script in the comments of `src/services/supabase.ts` to create the required tables and policies

### Google Calendar Integration Setup
To enable Google Calendar integration, you need to set up Google Sign-In for your Android app:
...
7. Update `android/build.gradle` to include the Google services classpath:
   ```gradle
   buildscript {
     dependencies {
   ...
   ```
8. Update `android/app/build.gradle` to apply the Google services plugin:
   ```gradle
